...
//                            TerminalAdd("\r\n" + rx_data + "\r\n> ");
    ...
    private void FileDownload(String param) {
        ...
        String cmd = "_dl=function() "
                + "  local ofile = io.open(\"" + DownloadedFileName + "\", \"r\")\n"
                + "  local buf "
                + "  local i=0 "
                + "  local checksum\n"
                + "  repeat "
 ...
 /* Snippets */
    ...
    private CompletionProvider createCompletionProvider() {
        ...
        if (OptionNodeMCU.isSelected()) {
            ...
            provider.addCompletion(new BasicCompletion(provider, "pwm.close(0)"));
            provider.addCompletion(new BasicCompletion(provider, "io.open(\"\",\"r\")"));
            provider.addCompletion(new BasicCompletion(provider, "io.write()"));
            provider.addCompletion(new BasicCompletion(provider, "io.close()"));
            provider.addCompletion(new BasicCompletion(provider, "file.remove()"));
            provider.addCompletion(new BasicCompletion(provider, "io.flush()"));
            provider.addCompletion(new BasicCompletion(provider, "io.seek()"));
    ...
    private boolean nodeSaveFileESP(String ft) {
        ...
        sendBuf.add("local ofile = io.open(\"" + ft + "\",\"w+\");");
        sendBuf.add("w = ofile:write('*line');\r\n");
        s = TextEditor1.get(iTab).getText().split("\r?\n");
        for (String subs : s) {
            sendBuf.add("w([==[" + subs + "]==]);");
        }
        sendBuf.add("ofile:flush();ofile:close();");
    ...
    private boolean nodeSaveFileESPTurbo(String ft) {
        ...
        sendBuf.add("local FILE=\"" + ft + "\" file.remove(FILE) local ofile = io.open(FILE,\"w+\") uart.setup(0," + Integer.toString(nSpeed) + ",8,0,1,0)");
        sendBuf.add("ESP_Receiver=function(rcvBuf) if string.match(rcvBuf,\"^ESP_cmd_close\")==nil then ofile:write(string.gsub(rcvBuf, \'\\r\', \'\')) uart.write(0, \"> \") else uart.on(\"data\") ");
        sendBuf.add("ofile:flush() ofile:close() FILE=nil rcvBuf=nil ESP_Receiver=nil uart.setup(0," + Integer.toString(nSpeed) + ",8,0,1,1) str=\"\\r\\n--Done--\\r\\n> \" print(str) str=nil collectgarbage() end end uart.on(\"data\",'\\r',ESP_Receiver,0)");
        int pos1 = 0;
        int pos2 = 0;
        int size = 254;
...
//        chooser.setMultiSelectionEnabled(false);
    ...
    private void UploadFilesStart() {
        ...
        String cmd = "_up=function(n,l,ll)\n"
                ...
                + "          i = i + 1\n"
                + "          local ofile = io.open(\"" + UploadFileName + "\",'a+')\n"
                + "          ofile:write(b)\n"
                + "          ofile:close()\n"
                + "          cs=0\n"
                + "          for j=1, l do\n"
    ...
    private void ViewFile(String fn) {
        String cmd = "_view=function()\n"
                + "local _line\n"
                + " local ofile = io.open(\"" + fn + "\",\"r\")\n" 
                + "if ofile then \n"
                + "    print(\"--FileView start\")\n"
                + "    repeat _line = ofile:read('*line') \n"
                + "        if (_line~=nil) then \n"
